...
public OnPlayerConnect(playerid)
{
    ...
    CheckPlayerForced(playerid);
	CheckPlayerHWID(playerid);
...
public ServerOnPlayerDeath(playerid, killerid, reason)
{
	...
	if(!Player[playerid][InDeathCamera])
	{
		...
		{
            GetPlayerPos(playerid, Player[playerid][LastPosX], Player[playerid][LastPosY], Player[playerid][LastPosZ]);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	    ...
	    if(response)
        {
            ...
            switch(listitem)
            {
                case 0:
                {
					...
					format(statsSTR[4], sizeof(statsSTR[]), ""COL_PRIM"- {FFFFFF}All Time Kills: \t\t%d\t\t"COL_PRIM"- {FFFFFF}All Time Deaths: \t\t%d "COL_PRIM"- {FFFFFF}All Time DMG: \t\t%d", Player[playerid][ATimeKills], Player[playerid][ATimeDeaths], Player[playerid][ATDMG]);
					new TotalStr[1400];
	...
	if(dialogid == DIALOG_REGISTER) {
	    if(response) {
			...
			new szHWID[CAC__MAX_HARDWARE_ID];
			if(CAC_GetStatus(playerid)) {
				// Get current HWID
				CAC_GetHardwareID(playerid, szHWID, CAC__MAX_HARDWARE_ID);
			}
			else {
				format(szHWID, CAC__MAX_HARDWARE_ID, "0");
			}

			new query[240];
			format(query, sizeof(query), "INSERT INTO Players (Name, Password, IP, LastSeen_Day, LastSeen_Month, LastSeen_Year, HWID) VALUES('%q', '%q', '%s', %d, %d, %d, '%s')", Player[playerid][Name], HashPass, IP, day, month, year, szHWID);
	        ...
	        switch(listitem)
			{
				...
				case 13:
				{
					...
					if(AntiCheatOnly == true) {
						AntiCheatOnly = false;
				    	format(iString, sizeof(iString), "{FFFFFF}%s "COL_PRIM"has {FFFFFF}disabled "COL_PRIM"SAMPCAC-only.", Player[playerid][Name]);
						SendClientMessageToAll(-1, iString);
					} else {
						AntiCheatOnly = true;
					    format(iString, sizeof(iString), "{FFFFFF}%s "COL_PRIM"has {FFFFFF}enabled "COL_PRIM"SAMPCAC-only.", Player[playerid][Name]);
				        SendClientMessageToAll(-1, iString);
					}
				    ShowConfigDialog(playerid);
				}
...
public e_COMMAND_ERRORS:OnPlayerCommandReceived(playerid, cmdtext[], e_COMMAND_ERRORS:success)
{
    ...
    if(Player[playerid][AllowCmds] == false) {
        SendErrorMessage(playerid, "You can't use any command.");
		return COMMAND_DENIED;
    }
    ...
    foreach(new i : Player)
	{
        if(Player[i][CommandsLog] == true && i != playerid){
            SendClientMessage(i, -1, sprintf("{FF6347}[*] {FFFFFF}%s "COL_PRIM"has typed {FFFFFF}%s", Player[playerid][Name], cmdtext));
        }
    }
...
YCMD:setref(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "set a player as referee.");
	    return 1;
	}
	new pID;
	if(sscanf(params, "d", pID)) return SendUsageMessage(playerid,"/setref [Player ID]");

	if(!IsPlayerConnected(pID)) return SendErrorMessage(playerid,"That player isn't connected.");
	if(Player[pID][Playing] == true) return SendErrorMessage(playerid, "That player is playing.");
	...
	SetPlayerSkin(pID, Skin[REFEREE]);
	ColorFix(pID);
    ...
    new iString[150];
	format(iString, sizeof(iString), "{FFFFFF}%s "COL_PRIM"has switched {FFFFFF}%s "COL_PRIM"to: {FFFFFF}%s", Player[playerid][Name], Player[pID][Name], TeamName[REFEREE]);
	SendClientMessageToAll(-1, iString);

    return 1;
}

YCMD:pswap(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "swap player's team.");
	    return 1;
	}
	...
	if(sscanf(params, "d", pID)) return SendUsageMessage(playerid,"/pswap [Player ID]");
    ...
    if(Player[pID][Team] == ATTACKER) {
        iTeam = DEFENDER;
    }
    else if(Player[pID][Team] == DEFENDER) {
        iTeam = ATTACKER;
    }
    else if(Player[pID][Team] == ATTACKER_SUB) {
        iTeam = DEFENDER_SUB;
    }
    else if(Player[pID][Team] == DEFENDER_SUB) {
        iTeam = ATTACKER_SUB;
    }
    else {
        return SendErrorMessage(playerid, "Can't swap a player that doesn't have a team.");
    }

	Player[pID][Team] = iTeam;
	SetPlayerSkin(pID, Skin[iTeam]);
	ColorFix(pID);
    ...
    UpdateRoundKillDmgTDmg(pID);
    new iString[150];
	format(iString, sizeof(iString), "{FFFFFF}%s "COL_PRIM"has switched {FFFFFF}%s "COL_PRIM"to: {FFFFFF}%s", Player[playerid][Name], Player[pID][Name], TeamName[iTeam]);
	SendClientMessageToAll(-1, iString);
...
YCMD:setsub(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "switch a player into the sub team.");
	    return 1;
	}
	new pID;
	if(sscanf(params, "d", pID)) return SendUsageMessage(playerid,"/setsub [Player ID]");
    ...
    if(Player[pID][Team] == ATTACKER) {
        ...
        return SendErrorMessage(playerid, "Player is already in sub team.");
    }
    else if(Player[pID][Team] == DEFENDER_SUB) {
        return SendErrorMessage(playerid, "Player is already in sub team.");
    }
...
YCMD:forcecac(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "force someone to use sampcac");
	    return 1;
	}

    new str[MAX_PLAYER_NAME];
	if(sscanf(params, "s", str)) return SendUsageMessage(playerid,"/forcecac [Account Name]");

    new iString[128];

	format(iString, sizeof(iString), "UPDATE Players SET AntiCheatForced = 1 WHERE Name = '%q'", str);
    db_free_result(db_query(sqliteconnection, iString));

    new pID = GetPlayerIdFromName(str);

    if(pID != INVALID_PLAYER_ID) {
        Player[pID][AntiCheatForced] = 1;

        if(!CAC_GetStatus(playerid)) {
            SendClientMessageToAll(-1, sprintf("{FFFFFF}%s "COL_PRIM"has been kicked from the server for being without {FFFFFF}SAMPCAC"COL_PRIM" while ac-forced.", Player[pID][Name]));
            SetTimerEx("OnPlayerKicked", 500, false, "i", pID);
        }
    } 

    SendClientMessage(playerid, -1, sprintf(""COL_PRIM"You have forced {FFFFFF}%s "COL_PRIM"to use anti-cheat.", str));
	return 1;
}

YCMD:unforcecac(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "unforce someone from using sampcac");
	    return 1;
	}

    new str[MAX_PLAYER_NAME];
	if(sscanf(params, "s", str)) return SendUsageMessage(playerid,"/unforcecac [Account Name]");

    new iString[128];

	format(iString, sizeof(iString), "UPDATE Players SET AntiCheatForced = 0 WHERE Name = '%q'", str);
    db_free_result(db_query(sqliteconnection, iString));

    SendClientMessage(playerid, -1, sprintf(""COL_PRIM"You have unforced {FFFFFF}%s "COL_PRIM"from using anti-cheat.", str));
	return 1;
}

YCMD:forcecaclist(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "display a list of all forced-cac players (offline and online).");
	    return 1;
	}
    new DBResult:res = db_query(sqliteconnection, "SELECT * FROM Players WHERE AntiCheatForced = 1 ORDER BY Name DESC");
    if(db_num_rows(res))
    {
		new holdStr[MAX_PLAYER_NAME];
		new bigStr[2048] = "Name\tForced\n";

		do
		{
            db_get_field_assoc(res, "Name", holdStr, sizeof(holdStr));
            format(bigStr, sizeof bigStr, "%s\n%s", bigStr, holdStr);
            db_get_field_assoc(res, "AntiCheatForced", holdStr, sizeof(holdStr));
		...
		ShowPlayerDialog(playerid, DIALOG_NO_RESPONSE, DIALOG_STYLE_TABLIST_HEADERS, "All Players Forced", bigStr, "Okay", "");
	}
	else
	{
	    ShowPlayerDialog(playerid, DIALOG_NO_RESPONSE, DIALOG_STYLE_MSGBOX, "All Players Forced", "No players forced...", "Okay", "");
	}
	return 1;
}
...
YCMD:showpm(playerid, params[], help)
{
	//if(Player[playerid][Level] < 1 && !IsPlayerAdmin(playerid)) return SendErrorMessage(playerid,"You need to be a higher admin level.");
    if(help)
	{
	    SendCommandHelpMessage(playerid, "display all the commands typed by any player.");
	    return 1;
	}
	new iString[128];

 	if(Player[playerid][bShowPM] == true) {
		Player[playerid][bShowPM] = false;
    	format(iString, sizeof(iString), "You have disabled "COL_PRIM"showpm.");
		SendClientMessage(playerid, -1, iString);

	} else {
		Player[playerid][bShowPM] = true;
	    format(iString, sizeof(iString), "You have enabled "COL_PRIM"showpm.");
		SendClientMessage(playerid, -1, iString);
	}
...
YCMD:showcmd(playerid, params[], help)
{
	//if(Player[playerid][Level] < 1 && !IsPlayerAdmin(playerid)) return SendErrorMessage(playerid,"You need to be a higher admin level.");
    if(help)
	{
	...
	new iString[128];
 	...
 	if(Player[playerid][CommandsLog] == true) {
		Player[playerid][CommandsLog] = false;
    	format(iString, sizeof(iString), "You have disabled "COL_PRIM"showcmd.");
		SendClientMessage(playerid, -1, iString);

	} else {
		Player[playerid][CommandsLog] = true;
	    format(iString, sizeof(iString), "You have enabled "COL_PRIM"showcmd.");
		SendClientMessage(playerid, -1, iString);
	}
...
YCMD:write(playerid, params[], help)
{
    //if(Player[playerid][Level] < 2 && !IsPlayerAdmin(playerid)) return SendErrorMessage(playerid,"You need to be a level 2 admin to do that.");
    if(help)
	{
	    SendCommandHelpMessage(playerid, "send a public message");
	    return 1;
	}
	if(isnull(params)) return SendUsageMessage(playerid,"/write [Text]");
	new iString[128];
	format(iString, sizeof(iString), "%s", params);
	SendClientMessageToAll(-1, iString);

	printf("%s (%d) used /write : %s", Player[playerid][Name], playerid, params);
    LogAdminCommand("write", playerid, INVALID_PLAYER_ID);
	return 1;
}
...
YCMD:createdm(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "create a new dm.");
	    return 1;
	}

    new Wep1[50], Wep2[50], Wep3[50];
	
    if(sscanf(params, "sss", Wep1, Wep2, Wep3)) {
        return SendUsageMessage(playerid,"/createmdm [weapon] [weapon] [weapon]");
    }

    new WeaponID1 = GetWeaponID(Wep1);
    new WeaponID2 = GetWeaponID(Wep2);
    new WeaponID3 = GetWeaponID(Wep3);

    if(WeaponID1 < 0 || WeaponID1 > 46 || WeaponID1 == 19 || WeaponID1 == 20 || WeaponID1 == 21 || WeaponID1 == 22) return SendErrorMessage(playerid, "Invalid Weapon Name.");
    if(WeaponID2 < 0 || WeaponID2 > 46 || WeaponID2 == 19 || WeaponID2 == 20 || WeaponID2 == 21 || WeaponID2 == 22) return SendErrorMessage(playerid, "Invalid Weapon Name.");
    if(WeaponID3 < 0 || WeaponID3 > 46 || WeaponID3 == 19 || WeaponID3 == 20 || WeaponID3 == 21 || WeaponID3 == 22) return SendErrorMessage(playerid, "Invalid Weapon Name.");

    if(TotalDMs > MAX_DMS)
	    return SendErrorMessage(playerid,"Too many DMs already created.");

    new DeathMatchId;
    DeathMatchId = FindFreeDMSlot();

    new Float:P[3], Position[128];
	GetPlayerPos(playerid, P[0], P[1], P[2]);
	format(Position, sizeof(Position), "%.0f,%.0f,%.0f", P[0], P[1], P[2]);

    new iString[256];
    format(iString, sizeof(iString), "INSERT INTO DMs (ID, Spawn, Interior, Wep1, Wep2, Wep3) VALUES (%d, '%s', %d, %d, %d, %d)", DeathMatchId, Position, GetPlayerInterior(playerid), WeaponID1, WeaponID2, WeaponID3);
	db_free_result(db_query(sqliteconnection, iString));
    
	SendClientMessageToAll(-1, sprintf("{FFFFFF}%s "COL_PRIM"has created DM ID: {FFFFFF}%d", Player[playerid][Name], DeathMatchId));
...
YCMD:deletedm(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "delete a dm.");
	    return 1;
	}

    new DeathMatchId;
	if(sscanf(params, "d", DeathMatchId)) return SendUsageMessage(playerid, "/deletedm [id]");

    if(DeathMatchId > MAX_DMS) return SendErrorMessage(playerid,"That DM doesn't exist.");
    if(!DMExist[DeathMatchId]) return SendErrorMessage(playerid,"That DM doesn't exist.");

    new iString[256];
    format(iString, sizeof(iString), "DELETE FROM DMs WHERE ID = %d", DeathMatchId);
    db_free_result(db_query(sqliteconnection, iString));

    DMExist[DeathMatchId] = false;

    SendClientMessageToAll(-1, sprintf("{FFFFFF}%s "COL_PRIM"has deleted DM ID: {FFFFFF}%d", Player[playerid][Name], DeathMatchId));
...
YCMD:mutecmd(playerid,params[], help)
{
	if(help)
	{
	    SendCommandHelpMessage(playerid, "mute-cmd a specific player.");
	    return 1;
	}
	new pID, iString[180];
    if(sscanf(params, "i", pID)) return SendUsageMessage(playerid,"/mutecmd [Player ID]");
	if(!IsPlayerConnected(pID)) return SendErrorMessage(playerid, "That player isn't connected.");

	if(Player[pID][AllowCmds] == false) return SendErrorMessage(playerid, "That player is already cmd-muted.");

	Player[pID][AllowCmds] = false;

	format(iString, sizeof(iString),"{FFFFFF}%s "COL_PRIM"has cmd-muted {FFFFFF}%s",Player[playerid][Name], Player[pID][Name]);
	
	SendClientMessageToAll(-1, iString);
    LogAdminCommand("mutecmd", playerid, pID);

	return 1;
}

YCMD:unmutecmd(playerid,params[], help)
{
	if(help)
	{
	    SendCommandHelpMessage(playerid, "unmute-cmd a specific player.");
	    return 1;
	}
	new pID, iString[180];
    if(sscanf(params, "i", pID)) return SendUsageMessage(playerid,"/unmutecmd [Player ID]");
	if(!IsPlayerConnected(pID)) return SendErrorMessage(playerid, "That player isn't connected.");

	if(Player[pID][AllowCmds] == true) return SendErrorMessage(playerid, "That player is not cmd-muted.");

	Player[pID][AllowCmds] = true;

	format(iString, sizeof(iString),"{FFFFFF}%s "COL_PRIM"has cmd-unmuted {FFFFFF}%s",Player[playerid][Name], Player[pID][Name]);
	
	SendClientMessageToAll(-1, iString);
    LogAdminCommand("unmutecmd", playerid, pID);
...
YCMD:astats(playerid,params[], help)
{
	...
	if(help)
	{
	    SendCommandHelpMessage(playerid, "get info of a specific player.");
	    return 1;
	}
	if(isnull(params)) return SendUsageMessage(playerid,"/astats [Player ID]");

	new iPlayerId = strval(params);
  	if(!IsPlayerConnected(iPlayerId)) return SendErrorMessage(playerid,"That player isn't connected.");

    new szHWID[CAC__MAX_HARDWARE_ID];
    CAC_GetHardwareID(iPlayerId, szHWID, CAC__MAX_HARDWARE_ID);

    new szIP[16];
    GetPlayerIp(iPlayerId, szIP, 16);

    new szCountry[64];
    GetPlayerCountry(iPlayerId, szCountry, sizeof(szCountry));

    new szGpci[41];
    gpci(iPlayerId, szGpci, sizeof(szGpci));

    SendClientMessage(playerid, -1, sprintf("{FFFFFF}%s's "COL_PRIM"informations: ", Player[iPlayerId][Name]));
    SendClientMessage(playerid, -1, sprintf(""COL_PRIM"HWID: {FFFFFF}%s", szHWID));
    SendClientMessage(playerid, -1, sprintf(""COL_PRIM"GPCI: {FFFFFF}%s", szGpci));
    SendClientMessage(playerid, -1, sprintf(""COL_PRIM"Country: {FFFFFF}%s", szCountry));
    SendClientMessage(playerid, -1, sprintf(""COL_PRIM"IP: {FFFFFF}%s", szIP));

 //   ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, sprintf("%s's stats", Player[iPlayerId][Name]), sprintf(""COL_PRIM"HWID\n{FFFFFF}%s\n\n"COL_PRIM"IP\n{FFFFFF}%s\n\n"COL_PRIM"Country\n{FFFFFF}%s\n\n"COL_PRIM"GPCI\n{FFFFFF}%s\n", szHWID, szIP, szCountry, szGpci), "Close", "");
...
YCMD:pm(playerid,params[], help)
{
    ...
    foreach(new i: Player) {
        if(Player[i][bShowPM] == true && playerid != i) {
            SendClientMessage(i, -1, sprintf("PM from "COL_PRIM"%s (%d) {FFFFFF}to "COL_PRIM"%s (%d) {FFFFFF}: %s", Player[playerid][Name], playerid, Player[recieverid][Name], recieverid, message));
        }
    }
...
YCMD:r(playerid,params[], help)
{
    ...
    foreach(new i: Player) {
        if(Player[i][bShowPM] == true && playerid != i) {
            SendClientMessage(i, -1, sprintf("PM from "COL_PRIM"%s (%d) {FFFFFF}to "COL_PRIM"%s (%d) {FFFFFF}: %s", Player[playerid][Name], playerid, Player[replytoid][Name], replytoid, params));
        }
    }
...
YCMD:alladmins(playerid, params[], help)
{
    ...
    if(db_num_rows(res))
    {
		...
		ShowPlayerDialog(playerid, DIALOG_NO_RESPONSE, DIALOG_STYLE_TABLIST_HEADERS, "All Server Admins", bigStr, "Okay", "");
	}
	else
	{
...
YCMD:aclist(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "display online players with AC.");
	    return 1;
	}
	...
	foreach(new i : Player) {
        if(CAC_GetStatus(i))
        {
            ...
            format(iString, sizeof(iString), "%s{FFFFFF}%s ({FF3333}v%d.%02d.%d{FFFFFF})\n", iString, Player[i][Name], major, minor, patch);
        }
	}

	if(strlen(iString) < 2) ShowPlayerDialog(playerid,DIALOG_NO_RESPONSE,DIALOG_STYLE_MSGBOX,"{FFFFFF}AC Players", "None is using SAMPCAC.","Ok","");
	else ShowPlayerDialog(playerid,DIALOG_NO_RESPONSE,DIALOG_STYLE_MSGBOX,"{FFFFFF}AC Players", iString, "Ok","");
...
YCMD:setaccname(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "change account's name.");
	    return 1;
	}

    new currentName[MAX_PLAYER_NAME], newName[MAX_PLAYER_NAME];

	if(Player[playerid][Logged] == false) return SendErrorMessage(playerid,"You must be logged in.");
	if(sscanf(params, "ss", currentName, newName)) return SendUsageMessage(playerid,"/setaccname [current name] [new name]");
	if(strlen(newName) < 3) return SendErrorMessage(playerid, "Username too short (Minimum 3 characters).");

	new iString[356];
	format(iString, sizeof(iString), "UPDATE Players SET Name = '%q' WHERE Name = '%q'", newName, currentName);
    db_free_result(db_query(sqliteconnection, iString));

	SendClientMessage(playerid, -1, sprintf("%s's name has been changed to: %s", currentName, newName));
	return 1;
}
YCMD:setaccpass(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "change account's pass.");
	    return 1;
	}
	...
	new iString[356];
	format(iString, sizeof(iString), "UPDATE Players SET Password = '%q' WHERE Name = '%q'", newName, currentName);
    db_free_result(db_query(sqliteconnection, iString));

	SendClientMessage(playerid, -1, sprintf("%s's password has been changed to: %s", currentName, newName));
	return 1;
}
...
YCMD:haka(playerid, params[], help) {
    if(help)
	{
	    SendCommandHelpMessage(playerid, "fetch HWID AKA data of a player.");
	    return 1;
	}
    ...
    new szString[256];
    format(szString, sizeof(szString), "SELECT * FROM Players WHERE Name = '%q'", Player[pID][Name]);
    new DBResult:res = db_query(sqliteconnection, szString);

    if(db_num_rows(res)) {
        new szHWID[64];
        db_get_field_assoc(res, "HWID", szHWID, sizeof(szHWID));

		if(strcmp(szHWID, "0", true) != 0) {
			format(szString, sizeof(szString), "SELECT * FROM Players WHERE HWID = '%q'", szHWID);
			new DBResult:result = db_query(sqliteconnection, szString);
			...
			if(db_num_rows(result)) {
				do
				{
					...
					db_get_field_assoc(result, "Name", szName, sizeof(szName));

					format(szBuffer, sizeof szBuffer, "%s%s\n", szBuffer, szName);
				}
				while(db_next_row(result));

				ShowPlayerDialog(playerid, 0, DIALOG_STYLE_LIST, "Players with the same HWID", szBuffer, "Close", "");
			}
		}
		else return SendErrorMessage(playerid, "That player hasn't a HWID associated.");
    }
    else {
        return SendErrorMessage(playerid, "Can't find that player in database.");
    }
    
    return 1;
}

YCMD:hbanlist(playerid, params[], help) {

    if(help)
	{
	    SendCommandHelpMessage(playerid, "HWID ban list.");
	    return 1;
	}

    new szString[256];
    format(szString, sizeof(szString), "SELECT * FROM HWID_bans");
    new DBResult:res = db_query(sqliteconnection, szString);
    ...
    if(db_num_rows(res)) {
		...
		do
		{
			...
			new szPlayerName[24];
			db_get_field_assoc(res, "Name", szPlayerName, sizeof(szPlayerName));

			format(szBuffer, sizeof szBuffer, "%s%s\t%s\n", szBuffer, szHWID, szPlayerName);
		}
		while(db_next_row(res));

		ShowPlayerDialog(playerid, 0, DIALOG_STYLE_TABLIST, "HWID bans", szBuffer, "Close", "");
        
    }
    else {
        return SendErrorMessage(playerid, "Can't find any ban.");
    }
    
    return 1;
}

YCMD:passaka(playerid, params[], help) {

    if(help)
	{
	    SendCommandHelpMessage(playerid, "fetch Passwords AKA data of a player.");
	    return 1;
	}
	new pID;
    if(sscanf(params, "u", pID)) {
    ...
    if(db_num_rows(res)) {
        new szPassword[64];
        db_get_field_assoc(res, "Password", szPassword, sizeof(szPassword));

        format(szString, sizeof(szString), "SELECT * FROM Players WHERE Password = '%q'", szPassword);
        new DBResult:result = db_query(sqliteconnection, szString);
        ...
        if(db_num_rows(result)) {
            ...
            ShowPlayerDialog(playerid, 0, DIALOG_STYLE_LIST, "Players with the same password", szBuffer, "Close", "");
        }
    }
    else {
        return SendErrorMessage(playerid, "Can't find that player in database.");
...
YCMD:searchacc(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "search account.");
	    return 1;
	}

	new szName[MAX_PLAYER_NAME];
	if(sscanf(params, "s", szName)) return SendUsageMessage(playerid,"/searchacc [Player Name / Part of Name]");

	new szString[256];
	format(szString, sizeof(szString), "SELECT * FROM `Players` WHERE Name LIKE '%%%q%%'", szName);
    new DBResult:res = db_query(sqliteconnection, szString);
	...
	if(db_num_rows(res)) {
		do
        {
            ...
            SendClientMessage(playerid, -1, sprintf(""COL_PRIM"_________ ACCOUNTS REGISTERED _________", szPlayerName));
			SendClientMessage(playerid, -1, sprintf(" || {FFFFFF}%s", szPlayerName));
		}
		while(db_next_row(res));
	}
...
YCMD:banhwid(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "ban hwid.");
	    return 1;
	}

	new szHWID[65];
	if(sscanf(params, "s", szHWID)) return SendUsageMessage(playerid,"/banhwid [HWID]");

	new szString[256];
	format(szString, sizeof(szString), "INSERT INTO HWID_bans (HWID) VALUES('%s')", szHWID);
    new DBResult:res = db_query(sqliteconnection, szString);

	SendClientMessage(playerid, -1, sprintf("You have banned "COL_PRIM"HWID: {FFFFFF}%s", szHWID));
...
YCMD:clearallhaka(playerid, params[], help)
{
	if(help)
	{
	    SendCommandHelpMessage(playerid, "clear all haka.");
	    return 1;
	}

	new szString[256];
	format(szString, sizeof(szString), "UPDATE Players SET HWID = '0'");
    new DBResult:res = db_query(sqliteconnection, szString);

	SendClientMessage(playerid, -1, ""COL_PRIM"Success. {FFFFFF}You have cleared all the haka.");
...
YCMD:clearhaka(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "clear someone's haka.");
	    return 1;
	}
	if(isnull(params) || !IsNumeric(params)) return SendUsageMessage(playerid,"/clearhaka [Player ID]");

	new pID = strval(params);
	if(!IsPlayerConnected(pID)) return SendErrorMessage(playerid,"That player is not connected.");

	new szString[256];
	format(szString, sizeof(szString), "UPDATE Players SET HWID = '0' WHERE Name = '%q'", Player[pID][Name]);
    new DBResult:res = db_query(sqliteconnection, szString);

	SendClientMessage(playerid, -1, ""COL_PRIM"Success. {FFFFFF}You have cleared the haka.");
...
YCMD:start(playerid, params[], help)
{
	...
	SendUsageMessage(playerid,"/start [base | arena | tdm | rc | last] [ID]");
	...
	else return SendUsageMessage(playerid,"/start [base | arena | tdm | rc | last] [ID]");
	...
	if(CommandID == 1) {
		...
		new TdmID = strval(Params[1]);

		if(TdmID > MAX_ARENAS) return SendErrorMessage(playerid,"That TDM does not exist.");
		if(!AExist[TdmID]) return SendErrorMessage(playerid,"That TDM does not exist.");

		AllowStartBase = false; // Make sure other player or you yourself is not able to start base on top of another base.
		GameType = TDM;
		SwitchGameType = true;

		SetTimerEx("OnArenaStart", 2000, false, "i", TdmID);

        new iString[144];
		format(iString, sizeof(iString), "{FFFFFF}%s "COL_PRIM"has started TDM: {FFFFFF}%s (ID: %d)", Player[playerid][Name], AName[TdmID], TdmID);
		SendClientMessageToAll(-1, iString);
	}
	skipped:
...
YCMD:forcesync(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "re-sync a player (restore stamina and other stuff).");
	    return 1;
	}
    
    new pID;

    if(sscanf(params, "d", pID)) return SendUsageMessage(playerid,"/forcesync [Player ID]");
	if(!IsPlayerConnected(pID)) return SendErrorMessage(playerid,"That player is not connected.");
    if(Player[pID][Spectating] == true) return SendErrorMessage(playerid,"That player is spectating someone.");

    SyncPlayer(pID);

    SendClientMessage(playerid, -1, sprintf("{FFFFFF}%s "COL_PRIM"has been synchronized", Player[pID][Name]));

    return 1;
}

YCMD:nclist(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "display netcheck-disabled players.");
	    return 1;
	}
	new iString[356] = '\0';

	foreach(new i : Player) {
	    if(Player[i][NetCheck] == 0) {
	    	format(iString, sizeof(iString), "%s{FFFFFF}%s\n", iString, Player[i][Name]);
		}
	}

	if(strlen(iString) < 2) ShowPlayerDialog(playerid,DIALOG_NO_RESPONSE,DIALOG_STYLE_MSGBOX,"{FFFFFF}Netcheck-disabled players", "No netcheck-disabled players.", "Ok","");
	else ShowPlayerDialog(playerid,DIALOG_NO_RESPONSE,DIALOG_STYLE_MSGBOX,"{FFFFFF}Netcheck-disabled players", iString,"Ok","");

	return 1;
}

YCMD:forcespecoff(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "remove a player from spectator mode.");
	    return 1;
	}
    
    new pID;

    if(sscanf(params, "d", pID)) return SendUsageMessage(playerid,"/forcespecoff [Player ID]");
	if(!IsPlayerConnected(pID)) return SendErrorMessage(playerid,"That player is not connected.");

    if(Player[pID][Spectating] == true || noclipdata[playerid][FlyMode] == true)
	{
 		StopSpectate(pID);
        SendClientMessage(playerid, -1, sprintf("{FFFFFF}%s "COL_PRIM"has been removed from spectator mode.", Player[pID][Name]));
		return 1;
	}
	else
	{
        SendClientMessage(playerid, -1,"{FFFFFF}Error: "COL_PRIM"He's not spectating anyone.");
	}
...
YCMD:fchat(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "fake-chat a player.");
	    return 1;
	}
    
    new pID, szMessage[128];
    if(sscanf(params, "ds", pID, szMessage)) return SendUsageMessage(playerid,"/fchat [Player ID] [Message]");
	if(!IsPlayerConnected(pID)) return SendErrorMessage(playerid,"That player is not connected.");

    OnPlayerText(pID, szMessage);
...
YCMD:tempadmin(playerid, params[], help)
{
	//if(Player[playerid][Level] < 5 && !IsPlayerAdmin(playerid)) return SendErrorMessage(playerid,"You need to be level 5 or rcon admin.");
	if(help)
	{
	...
	format(iString,sizeof(iString),"{FFFFFF}\"%s\" "COL_PRIM"has temporary set {FFFFFF}\"%s\"'s "COL_PRIM"level to: {FFFFFF}%d", Player[playerid][Name], Player[GiveID][Name], LEVEL);
	SendClientMessageToAll(-1, iString);
    LogAdminCommand("tempadmin", playerid, GiveID);
	return 1;
}
...
public OnScriptUpdate()
{
    ...
    foreach(new i : Player)
	{
		...
		{
  			PlayerTextDrawSetString(i, FPSPingPacket[i], sprintf("~y~PING~w~: %d  ~y~FPS~w~: %d  ~y~PL~w~: %.1f%%", GetPlayerPing(i), Player[i][FPS], NetStats_PacketLossPercent(i)));
